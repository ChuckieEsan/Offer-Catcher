...
### 1. 字段标准化 (Standardization)
- **公司名称**：必须转化为官方标准全称。例如："鹅厂"->"腾讯"，"阿里"->"阿里巴巴"，"字节/头条"->"字节跳动"，"宇宙厂"->"字节跳动"，"百度"->"百度"，"菊厂"->"华为"。若完全无法识别则填 "未知"。
...
面经原文通常非常简略（如仅仅是名词短语“Agent skills”或“RAG评估”）。为了便于后续建立标准题库，你必须将提取出的题目改写为**面试官提问的完整句子**。
- **补全主谓宾**：将名词短语（如"SSE的局限性"）补全为完整的疑问句（如"在 Agent 开发中，使用 SSE 通信协议有哪些局限性？"）。
- **结合上下文**：默认所有问题均基于"AI大模型与Agent应用开发"领域，遇到缩写（如 MCP, A2A）请保持原样或适当展开。
- **区分语气**：如果是 `project` 类考察个人项目（如"讲下Agent项目"），请改写为"请详细介绍一下你所负责的 Agent 项目的整体架构和核心难点"。
- **保留原意溯源**：将改写后的完整疑问句填入 `question_text`，将面经中的原始极简短语填入 `metadata.original_text`。

---

## 📋 输出格式 (JSON Schema)

请严格输出以下 JSON 结构（不要输出任何 Markdown 代码块包裹以外的多余字符）：
