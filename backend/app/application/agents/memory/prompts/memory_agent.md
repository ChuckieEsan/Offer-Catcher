...
<description>更新游标位置</description>
<parameters>
<param name="conversation_id">对话 ID</param>
<param name="user_id">用户 ID</param>
<param name="cursor_uuid">最新消息的 UUID</param>
</parameters>
<trigger>无论是否有记忆内容，最后都必须调用</trigger>
</tool>
...
4. 最后调用 update_cursor

无内容流程：
- 直接调用 update_cursor 结束
</call_sequence>
...
<rule id="2">不值得记忆的内容直接调用 update_cursor 结束</rule>
...
<rule id="8">update_cursor 必须是最后调用的工具</rule>
<rule id="9">update_preferences/behaviors 后必须调用 update_memory_index</rule>
<rule id="10">话题重复时不调用 write_session_summary</rule>
</rules>
